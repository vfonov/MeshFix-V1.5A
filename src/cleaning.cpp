...
int ExtTriMesh::swapAndCollapse()
{
 ...
 {
  UNMARK_VISIT(t);
  if (isDegenerateTriangle(t)||t->isDegenerate()) {triangles.appendTail(t); MARK_VISIT2(t);} // in rare cases isDegenerateTriangle and t->isDegenerate differ, so that tin.clean did not converge
 }
...
// modified cleaning; respects "protection" bit; returns true on success
bool ExtTriMesh::cleanP(int max_iters, int inner_loops, int number_components_to_keep)
{
 bool ni, nd;
