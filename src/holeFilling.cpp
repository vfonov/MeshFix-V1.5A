...
Edge *ExtTriMesh::joinBoundaryLoops(Vertex *gv, Vertex *gw, bool justconnect, bool refine, bool fair)
{
 ...
 // A.T.: outdated; better use joinBoundaryLoops2
 ...
 v=gv; do {e=v->nextBoundaryEdge(); v=e->oppositeVertex(v); tl1+=e->length();} while (v!=gvn);
 v=gw; do {e=v->prevBoundaryEdge(); v=e->oppositeVertex(v); tl2+=e->length();} while (v!=gwn); //A.T.: bug fixed; otherwise tl1==tl2;
 pl1=tl1; pl2=tl2;
...
Edge *ExtTriMesh::joinBoundaryLoops2(Vertex *gv, Vertex *gw, bool refine, bool fair)
{
 Vertex *v, *w, *gvn, *gwn, *gvStop, *gwStop, *closestPartner;
 Edge *e, *eHlp, *gve, *gwe;
 Triangle *t;
 Node *n;
 double tl1=0.0, tl2=0.0, pl1, pl2;
 double c1, c2;
 double adist, mindist2, mindist1, pl;

 // remark: more stable with refine=false, fair not tested

 JMesh::info("USING joinBoundaryLoops2\n");
 ...
 // test whether there are indeed two seperate loops
 v=gv;
 do {v=v->nextOnBoundary(); if (v==gw) return NULL;} while (v!=gv);

 // insert two initial triangles
 gve = gv->prevBoundaryEdge();
 gvn = gve->oppositeVertex(gv);
 gwe = gw->nextBoundaryEdge();
 gwn = gwe->oppositeVertex(gw);
 ...
 // get path length for each loop
 v=gv; do {e=v->nextBoundaryEdge(); v=e->oppositeVertex(v); tl1+=e->length(); } while (v!=gvn);
 v=gw; do {e=v->prevBoundaryEdge(); v=e->oppositeVertex(v); tl2+=e->length(); } while (v!=gwn);


 // loop to insert other triangles
 e=je;
 gvStop = gvn;
 gwStop = gwn;
 while (e->isOnBoundary())
 {
  ...
  // get minimal distance from gv to remaining vertices of other loop
  w=gwn;
  mindist1=gv->squaredDistance(w);
  pl=0.0;
  while ((pl<0.5*tl2) && (w!=gwStop)) {
      eHlp=w->prevBoundaryEdge(); w=eHlp->oppositeVertex(w);
      pl+=eHlp->length();
      if ((adist = gv->squaredDistance(w)) < mindist1) mindist1=adist;
  }

  // get minimal distance from gw to remaining vertices of other loop
  v=gvn;
  mindist2=gw->squaredDistance(v);
  pl=0.0;
  while ((pl<0.5*tl1) && (v!=gvStop)) {
      eHlp=v->nextBoundaryEdge(); v=eHlp->oppositeVertex(v);
      pl+=eHlp->length();
      if ((adist = gw->squaredDistance(v)) < mindist2) mindist2=adist;
  }

  // insert new triangle, depending on minimal distance
...
// alternative version using only local distance information
/*
e=je;
double c1, c2;
while (e->isOnBoundary())
{
 ...
 c1 = gv->squaredDistance(gwn);
 c2 = gw->squaredDistance(gvn);
