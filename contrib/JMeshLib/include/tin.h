...
class Triangulation
{
 ...
 int saveMSH(const char *, bool save_masks = 0, char maskByte = 0, bool save_normals = 0, bool comb_separVertices = 0);             //!< Saves gmsh MSH format
 int saveFSMESH(const char *, float xshift = 0.0 );          //!< Saves FreeSurfer format
 int saveVTK(const char *); //!< Saves VTK format
 ...
 //! Creates a new Triangulation out of a selection for usage as separation surface;

 //! triangles and edges are duplicated, vertices are NOT duplicated,
 //! e0 edges of vertices are NOT changed
 Triangulation *createSeparSurfFromSelection();
 ...
 void   unmarkEverything(char maskByte = 0);
