...
// Creates a new mesh out of a selection using the existing vertices
Triangulation *Triangulation::createSeparSurfFromSelection()
{
    Triangle *t,*s, *nt;
    Node *n;
    ...
    FOREACHEDGE(e, n) if (IS_BIT(e,3))
      {sE.appendHead(e); MARK_BIT(e->v1, 3); MARK_BIT(e->v2, 3);}
    //FOREACHVERTEX(v, n) if (IS_BIT(v,3)) sV.appendHead(v);
    FOREACHVERTEX(v, n) if (IS_BIT(v,3)) { sV.appendHead(v); tin->V.appendTail(v);}
 ...
 JMesh::info("a1\n");
   ...
   // FOREACHVVVERTEX((&sV), v, n)
    ...
    FOREACHVEEDGE((&sE), e, n)
     {ne=new Edge(e->v1, e->v2); tin->E.appendTail(ne); e->info = ne;}
...
JMesh::info("a2\n");
    //i=0; if (!keep_ref) FOREACHVVVERTEX((&sV), v, n) v->info = v_info[i++];
    i=0; FOREACHVEEDGE((&sE), e, n) e->info = e_info[i++];
    i=0; FOREACHVTTRIANGLE((&sT), t, n) t->info = t_info[i++];
...
void Triangulation::unmarkEverything(char maskByte)
{
 Vertex *v;
 Edge *e;
 Triangle *t;
 ...
 if(!maskByte) {
    ...
    for(i=0; i<8; i++)
        if(!(maskByte & 1<<i)) {
            FOREACHVERTEX(v, n) UNMARK_BIT(v,i);
            FOREACHEDGE(e, n) UNMARK_BIT(e,i);
            FOREACHTRIANGLE(t, n) UNMARK_BIT(t,i);
        }
 }
}
