...
class ExtTriMesh : public Triangulation
{
 ...
 Edge	*joinBoundaryLoops2(Vertex *, Vertex *, bool =1, bool =1); // (in "ALGORITHMS/holeFilling.cpp")
 ...
 // Mirko's and Axel's functions
 int  joinOverlappingComponentPair();
 int  joinOverlappingComponentPair2(bool firstIn = false, bool secondIn = false);
 ...
 bool decoupleFirstFromSecondComponent(double minAllowedDistance = 1.0, unsigned max_iterations = 10, bool treatFirstAsOuter = true, bool outwards = true, double stepWidth = 0.5);
 ...
 //! Refines all triangles of the first component that are inside the second component.
 void refineFirstWithinSecondComponent(double factor = 1.0);
 //! Refines selected triangles
 int refineSelectedTriangles(double factor = 1.0);
 ...
 //! applies a 4x4 transformation matrix to the surface mesh
 bool applyTrafo(const char*);
 ...
 bool cleanP(int max_iters = 10, int inner_loops = 3, int number_components_to_keep = 1);
 ...
 int taubinSmooth(int = 1, double = 0.5);
 int removeSpikes(double = 5.5);
