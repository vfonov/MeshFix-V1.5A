...
void usage()
{
 ...
 printf(" --j-outout          Join the 2 biggest components if they overlap, remove the rest \n");
 printf("                     (including the surface parts inside each other.)\n");
 printf(" --j-inin            Join the inner parts of the 2 biggest components if they overlap.\n");
 printf(" --j-outin           Join the outer part of the first component with the inner of the second component.\n");
 printf(" -j                  Same as --j-outout\n");
 printf(" -jc                 Join the closest pair of components.\n");
 printf(" -h, --help          Print this help and exit.\n");
 printf(" --shells <n>        Only the n biggest shells are kept.\n");
 printf(" --splitShells       Save each shell in own file, sorted by number of triangles (only .off and .stl)\n");
 printf("   --shellMinArea <a> Only shells with an area > a are kept when using --splitShells (the maximal number of\n");
 printf("                     shells is given by --shells, i.e. both have to be set)\n");
 ...
 printf("   --vertexDensity <d> Constrain density of vertices to d (overwrites --vertices)\n");
 printf(" --no-clean          Don't clean.\n");
 printf(" --smooth <n>        Apply n laplacian smoothing steps.\n");
 printf(" --taubin <n>        Apply n taubin smoothing steps.\n");
 printf(" --removeSpikes <t>  Removes spikes by deleting nodes with gaussian curvature <= t. The curvature is the total sum of incoming angles, in radians. A good value seems to be 5.5\n");
 printf(" --trafo <tfile>     Apply 4x4 transformation matrix save in tfile (simple text file with 4 lines)\n");
 printf("                     as last step before writing the output mesh\n");
 ...
 printf(" --vtk               Result is saved in VTK     format instead of OFF.\n");
 ...
 printf(" --refine-inner      Refines the triangles of component 1 that are inside component 2,\n");
 ...
 printf(" --step <d>          Set step-size by which triangles are moved with --decouple-inin, \n");
 printf("                     --decouple-outin or --decouple-outout (standard: 0.5) \n");
 ...
 printf(" --fineTuneOut <d> <n> Similar to --fineTuneIn, but ensures minimal distance \n");
 printf("                     in the other direction\n");
...
int main(int argc, char *argv[])
{
 ...
 bool joinOverlappingComponentsInIN = false;
 bool joinOverlappingComponentsOutIN = false;
 bool attachTo = false;
...
// float minAllowedDistance = 0;
 ...
 int taubinSmoothingSteps = 0;
 double removeSpikes = -1;
 double cutOuterMinDist = -1, cutInnerMinDist = -1;
 double refineInnerFactor = -1;
 double decoupleOuterOutMinDist = -1, decoupleOuterInMinDist = -1, decoupleInnerInMinDist = -1;
 double decoupleStepWidth = 0.5;
 double dilateDist = 0;
 double fineTuneIn = -1, fineTuneOut = -1;
 double shellMinArea = 0;
 double vertexDensity = 0;
 bool splitShells = false;
 bool clean = true;
 ...
 for (int i=2; i<argc; i++)
 {
  ...
  else if (!strcmp(argv[i], "--vtk")) save_vtk = true;
  else if (!strcmp(argv[i], "-j") || !strcmp(argv[i], "--j-outout")) joinOverlappingComponents = true;
  else if (!strcmp(argv[i], "--j-inin")) joinOverlappingComponentsInIN = true;
  else if (!strcmp(argv[i], "--j-outin")) joinOverlappingComponentsOutIN = true;
  else if (!strcmp(argv[i], "--attachTo")) attachTo = true;
  else if (!strcmp(argv[i], "--splitShells")) splitShells = true;
  ...
  else if (!strcmp(argv[i], "--taubin")) {
      if (i>=argc-1 || (taubinSmoothingSteps = atoi(argv[i+1]))<0)
          JMesh::error("# Taubin smoothing steps must be >= 1.\n");
      i++;
  }
  else if (!strcmp(argv[i], "--removeSpikes")) {
      if (i>=argc-1) JMesh::error("error reading removeSpikes\n");
      removeSpikes = (float) atof(argv[i+1]);
      JMesh::info("spike removal threshold set to %f\n", removeSpikes);
      i++;
  }
  ...
  else if (!strcmp(argv[i], "--refine-inner")) {
     if (i<argc-1) {
         refineInnerFactor = atof(argv[i+1]);
         if (refineInnerFactor < 1.0)
             JMesh::error("refinement factor must be >= 1.0.\n");
         else
             i++;
     }
  }
      ...
      if (i<argc-1) {
          decoupleStepWidth = atof(argv[i+1]);
          if (decoupleStepWidth <= 0)
              JMesh::error("step width must be > 0.\n");
          else
              i++;
      }
  }
  else if (!strcmp(argv[i], "--shellMinArea")) {
      if (i<argc-1) {
          shellMinArea = atof(argv[i+1]);
          if (shellMinArea <= 0)
              JMesh::error("shellMinArea must be > 0.\n");
          else
              i++;
      }
  }
  else if (!strcmp(argv[i], "--vertexDensity")) {
      if (i<argc-1) {
          vertexDensity = atof(argv[i+1]);
          if (vertexDensity <= 0)
              JMesh::error("vertexDensity must be > 0.\n");
          else
              i++;
      }
  }
 ...
 if (joinOverlappingComponents || joinOverlappingComponentsInIN || joinOverlappingComponentsOutIN) {
     bool firstIn = false, secondIn = false;
     if(joinOverlappingComponentsInIN)  firstIn = secondIn = true;
     if(joinOverlappingComponentsOutIN) secondIn = true;

     tin.removeSmallestComponents(2);
     if(!tin.joinOverlappingComponentPair2(firstIn,secondIn)) {
 ...
 /*
 if (attachTo) {
     printf("\nAttaching outer part of second component to first component ...\n");
 ...
 */
 ...
 if (refineInnerFactor >= 0) {
     printf("Refining triangles of the first component, that are inside of the second one.\n");
     if(tin.shells() != 2) JMesh::warning("Incorrect number of components, won't cut. Having %d and should have 2.\n", tin.shells());
     else tin.refineFirstWithinSecondComponent(refineInnerFactor);
 }
 if (decoupleOuterOutMinDist >= 0) {
     ...
     else tin.decoupleFirstFromSecondComponent(decoupleOuterOutMinDist, 15, true, true, decoupleStepWidth);
     ...
     if(tin.shells() != 2) JMesh::warning("Incorrect number of components, won't decouple. Having %d and should have 2.\n", tin.shells());
     else tin.decoupleFirstFromSecondComponent(decoupleOuterInMinDist, 15, true, false, decoupleStepWidth);
     numberComponentsToKeep = 1; // for subsequent cleaning
 } else if(decoupleInnerInMinDist >= 0) {
     if(numberComponentsToKeep == 1) JMesh::warning("Use --shells 2 for decoupling.\n");
     printf("Decoupling first (inner) component from second one (move inwards). Min. distance: %g.\n", decoupleInnerInMinDist);
     if(tin.shells() != 2) JMesh::warning("Incorrect number of components, won't decouple. Having %d and should have 2.\n", tin.shells());
     else tin.decoupleFirstFromSecondComponent(decoupleInnerInMinDist, 15, false, false, decoupleStepWidth);
     numberComponentsToKeep = 1; // for subsequent cleaning
 }
 ...
 if (uniformRemeshSteps) {
     ...
     if(vertexDensity>0 && numberOfVertices) JMesh::warning("--vertexDensity overwrites --vertices.");

     if(vertexDensity>0) {
         numberOfVertices=(int) tin.area()*vertexDensity;
         JMesh::info("area %.0f; given density %.2f; number of vertices %d\n",tin.area(),vertexDensity,numberOfVertices);
     }

     tin.uniformRemesh(uniformRemeshSteps, numberOfVertices, tin.E.numels());
 } else {
     if(numberOfVertices) JMesh::warning("--vertices works only together with -u.");
     if(vertexDensity>0) JMesh::warning("--vertexDensity works only together with -u.");
 }

 if(smoothingSteps) {
     printf("Smoothing %d steps.\n", smoothingSteps);
     tin.laplacianSmooth(smoothingSteps, 1.0);
 }

 if(taubinSmoothingSteps){
     printf("Smoothing %d steps.\n", taubinSmoothingSteps);
     tin.taubinSmooth(taubinSmoothingSteps, 0.5);
 }

 if(removeSpikes >= 0){
     printf("Removing Spikes.\n");
     tin.removeSpikes(removeSpikes);
 }
 ...
 if (haveIntersectText) {
     ...
     if(haveOutputFile) tin.saveMSH(outputFile,1,1,1);
 ...
 if (applyTrafo) {
     printf("Applying transformation %s ...\n",trafoFile);
     tin.applyTrafo(trafoFile);
 }

 if (!splitShells) {
    char *fname = createFilename( haveOutputFile ? outputFile : argv[1], haveOutputFile ? "": "_fixed", (save_vrml? ".wrl" : (save_stl? ".stl": (save_msh? ".msh" : (save_fsmesh? ".fsmesh" : (save_vtk? ".vtk" : ".off"))))), !haveOutputFile);
    ...
    // save each shell as separate file (supports only off and stl format)
    ExtTriMesh *shellPtr;
    int shellidx=1;
    char hlpstr[15];
    std::multimap<const unsigned, List*, std::greater<const unsigned> > sizeListMap;
    List *components = tin.getComponentsList();
    ...
    for(; it != sizeListMap.end(); it++) {
        List *l = it->second;
        Triangle *t = (Triangle*) l->popHead();
        shellPtr = (ExtTriMesh*) tin.extractShell(t);
        if( shellMinArea>0 && shellPtr->area()<=shellMinArea) continue;

        sprintf(hlpstr, "%s%d", haveOutputFile ? "": "_fixed", shellidx);
        char *fname = createFilename( haveOutputFile ? outputFile : argv[1], hlpstr,save_stl? ".stl": ".off", !haveOutputFile);
        if (save_stl)
            shellPtr->saveSTL(fname);
        else
            shellPtr->saveOFF(fname);
        printf("Saving output mesh to %s\n",fname);
        shellidx++;
    }
 }
